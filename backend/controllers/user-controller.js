...
export const getSuggestedUsers = async (req,res) =>{
    ...
    try{
        ...
        //shouldnt see passwords for suggested users
    ...
    try{
        ...
        if(currentPassword && newPassword){
            ...
            if(newPassword.length < 6){
                return res.status(400).json({ message: "New Password must be 6 characters or greater"});
            }
        ...
        if(coverImg){
            if(user.coverImg){
                await cloudinary.uploader.destroy(user.covereImg.split("/").pop().split(".")[0]);
            }
